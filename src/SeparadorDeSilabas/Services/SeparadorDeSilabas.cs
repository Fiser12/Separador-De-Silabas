...
namespace SeparadorFrases
{
    class SeparadorDeSilabas
    {
        ...
        public ArrayList Silabas(String palabra)
        {
            ArrayList silabas = new ArrayList();
            int numeroSilabas = NumeroSilabas(palabra);
            ArrayList posiciones = PosicionSilabas(palabra);
            for(int i = 0; i < numeroSilabas; i++)
            {
                if (i +1 <  numeroSilabas && i + 1 < posiciones.Count)
                {
               /*     Console.WriteLine(posiciones.Count + " " );
                    Console.WriteLine(palabra.Substring((int) posiciones[i]) + " " + posiciones[i]);
                    Console.WriteLine(palabra.Substring((int)posiciones[i + 1]) + " " + posiciones[i+1]);
                    Console.WriteLine(palabra.Substring((int)posiciones[i], (int)posiciones[i + 1]));*/
                    string temp = palabra.Substring((int) posiciones[i], (int) posiciones[i + 1] - (int)posiciones[i]);
                    silabas.Add(temp);
                }
                else{
                    String temp = palabra.Substring((int) posiciones[i]);
                    silabas.Add(temp);
                }
            }
            return silabas;
